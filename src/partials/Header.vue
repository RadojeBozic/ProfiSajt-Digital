...
// ---------- AUTH STANJE (localStorage) ----------

// inicijalno očitavanje iz localStorage
const currentName = ref(localStorage.getItem('ps_user_name') || null)
const hasToken = ref(!!localStorage.getItem('ps_token'))

// helper – da imamo centralno mesto
const syncAuthFromStorage = () => {
  currentName.value = localStorage.getItem('ps_user_name') || null
  hasToken.value = !!localStorage.getItem('ps_token')
}

// ako se token promeni iz drugih tabova, sync-ujemo
if (typeof window !== 'undefined') {
  window.addEventListener('storage', (event) => {
    if (event.key === 'ps_token' || event.key === 'ps_user_name') {
      syncAuthFromStorage()
    }
  })
}

// mali helper za template
const isAuthenticated = () => hasToken.value

// logout – brišemo token + ime, sync i redirect
const handleLogout = () => {
          ...
          >
            ...
            >
              ...
              <div class="p-2 space-y-1">
                ...
                <template v-if="!isAuthenticated()">
                  <router-link
                    class="block text-gray-700 hover:bg-gray-100 rounded-lg py-1.5 px-2 transition"
                    to="/signin"
                    @click="mobileNavOpen = false"
                  >
                    {{ t('nav.login') }}
                ...
                <template v-else>
                  ...
                  <button
                    @click="() => { handleLogout(); mobileNavOpen = false }"
                    class="block w-full text-left text-red-600 hover:bg-red-50 rounded-lg py-1.5 px-2 transition"
                  >
                    {{ t('nav.logout') || 'Logout' }}
                  </button>
                </template>
